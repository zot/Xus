        ...
        return peer.addHandler('this/public/storage', {
          get: function(_arg, errBlock, cont) {
            ...
            if (m = key.match(new RegExp('peer/[^/]+/public/storage(/(.*)|)$'))) {
              pth = path.normalize("" + dir + "/" + ((_ref = m[2]) != null ? _ref : ''));
              return fs.stat(pth, function(err, stats) {
                if (err) {
                  return errBlock('error_bad_peer_request', "No such file or directory: " + pth);
                } else if (stats.isDirectory()) {
                  return fs.readdir(pth, function(err, files) {
                    var readNext, result;
                    if (err) {
                      return errBlock('error_bad_peer_request', "Could not read directory: " + pth);
                    } else {
                      i = 0;
                      result = {};
                      readNext = function() {
                        ...
                        if (i < files.length) {
                          ...
                          return fs.stat(child, function(err, stats) {
                            if (err) {
                              return errBlock('error_bad_peer_request', "Error statting child " + pth + ": " + err);
                            } else {
                              result[child] = {
          ...
          set: function(_arg, errBlock, cont) {
            ...
            if (m = key.match(new RegExp('peer/[^/]+/public/storage/(.+)$'))) {
              ...
              return errBlock('error_bad_peer_request', "File retrieval not supported, yet: " + m[1]);
            }
          }
        });
      });
