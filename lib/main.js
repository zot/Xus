...
(function() {
  ...
  run = function() {
    console.log("Xus args: " + (JSON.stringify(process.argv)));
    return setup(function(s) {
      ...
      if (state.servers[config.name]) {
        ...
        httpServer = startWebSocketServer(config, function() {
          ...
          if (config.cmd) {
            console.log("Server command: " + config.cmd);
          }
        ...
        return (config.proxy ? startProxy : startXus)(config, httpServer, function(master) {
          ...
          console.log("requiring: " + (JSON.stringify(requirements)));
          _results = [];
