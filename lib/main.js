// Generated by CoffeeScript 1.3.3
(function() {
  var Server, exports, path, pfs, run, setup, startSocketServer, stateFd, usage, _ref;

  _ref = exports = module.exports = require('./socket'), Server = _ref.Server, startSocketServer = _ref.startSocketServer;

  pfs = require('./pfs');

  path = require('path');

  usage = function(args) {
    console.log("Usage: node " + args[1] + " name [-p port]");
    return process.exit();
  };

  stateFd = null;

  setup = function(cont) {
    var stateFile, xusDir;
    try {
      xusDir = path.join(process.env.HOME, '.xus');
      stateFile = path.join(xusDir, 'state');
      return pfs.stat(xusDir).fail(pfs.mkdir(xusDir)).then(pfs.open(stateFile, 'r+')).then(function(fd) {
        return pfs.flock((stateFd = fd), 'ex');
      }).then(function() {
        return pfs.readFile(stateFd);
      }).then(function(s) {
        return (cont || function() {})(s);
      }).end();
    } catch (err) {
      return console.log("Error: " + err.stack);
    }
  };

  run = function() {
    console.log("ARGS: " + (process.argv.join(' ')));
    return setup(function(s) {
      var args, host, i, name, port, state, xusServer;
      state = (s && JSON.parse(s)) || {
        servers: {}
      };
      console.log("State: '" + (JSON.stringify(state)) + "'");
      i = 2;
      args = process.argv;
      if (i > args.length) {
        usage(args);
      }
      name = args[1];
      while (i < args.length) {
        switch (args[i]) {
          case '-p':
            port = args[++i];
            break;
          case '-h':
            host = args[++i];
        }
        i++;
      }
      xusServer = new Server();
      return startSocketServer(xusServer, port, host, function() {
        console.log("Server " + name + " started on port: " + (xusServer.socketServer.address().port));
        state.servers[name] = {
          address: xusServer.socketServer.address()
        };
        return pfs.truncate(stateFd, 0).then(pfs.writeFile(stateFd)).then(pfs.close(stateFd));
      });
    });
  };

  exports.run = run;

  exports.setup = setup;

}).call(this);
